...
// Fetch expense data from the database with pagination
$sql = "SELECT e.*, et.title AS expense_title FROM expense e JOIN expense_type et ON e.expense_type = et.id ORDER BY e.id DESC LIMIT ?, ?";
$stmt = $conn->prepare($sql);
...
$sql = "SELECT COUNT(id) FROM expense";
$result = $conn->query($sql);
$row = $result->fetch_row();
$total_records = $row[0];
$total_pages = ceil($total_records / $limit);

// Function to fetch expense types from the database
function getExpenseTypes($conn) {
    $expenseTypes = [];
    $query = "SELECT id, title FROM expense_type ORDER BY title ASC";
    if ($result = $conn->query($query)) {
        ...
        // Handle query error
        die("Error fetching expense types: " . $conn->error);
    }
    return $expenseTypes;
}
...
@media (max-width: 768px) {
        ...
        <div class="sidebar-body pt-0 data-scrollbar">
            <div class="sidebar-list">
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="./expense_record.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
           ...
           <table class="table table-bordered table-hover" id="expenseTable">
                            ...
                            <tbody>
                                <?php foreach ($expenses as $expense): ?>
                                    <tr id="expense-<?= htmlspecialchars($expense['id'], ENT_QUOTES, 'UTF-8') ?>">
                                        <td><?= htmlspecialchars($expense['id'], ENT_QUOTES, 'UTF-8') ?></td>
                                        <td>
                                            <?php if ($role === 'admin'): ?>
                                            <button class="btn btn-sm btn-success edit-btn" data-id="<?= htmlspecialchars($expense['id'], ENT_QUOTES, 'UTF-8') ?>">
                                                <i class="fas fa-edit"></i> Edit
                                            </button>
                                            <button class="btn btn-sm btn-danger delete-btn" data-id="<?= htmlspecialchars($expense['id'], ENT_QUOTES, 'UTF-8') ?>">
                                                <i class="fas fa-trash-alt"></i> Delete
                                            </button>
                                            <?php endif; ?>
                                        </td>
                                        <td><?= htmlspecialchars($expense['expense_title'], ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?= htmlspecialchars($expense['payment_method'], ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?= htmlspecialchars($expense['cost'], ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?= htmlspecialchars($expense['paid_to'], ENT_QUOTES, 'UTF-8') ?></td>
                                        <td><?= htmlspecialchars($expense['tin_number'], ENT_QUOTES, 'UTF-8') ?></td>
 ...
 <div class="modal fade" id="addExpenseModal" tabindex="-1" aria-labelledby="addExpenseModalLabel" aria-hidden="true">
                                        ...
                                        <select class="form-select" id="expenseType" name="expense_type" required>
                                            ...
                                            <?php foreach ($expenseTypes as $type): ?>
                                                <option value="<?= htmlspecialchars($type['id'], ENT_QUOTES, 'UTF-8') ?>">
                                                    <?= htmlspecialchars($type['title'], ENT_QUOTES, 'UTF-8') ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                        ...
                        <form id="editExpenseForm">
                            ...
                            <div class="container-fluid">
                                <div class="row">
                                    <!-- Expense Type -->
                                    <div class="col-md-6 mb-3">
                                        <label for="edit_expenseType" class="form-label">Expense Type</label>
                                        <select class="form-select" id="edit_expenseType" name="expense_type" required>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select" id="edit_paymentMethod" name="payment_method" required>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="number" class="form-control" id="edit_cost" name="cost" required step="0.01">
                                    </div>
                                </div>
                                <div class="row">
                                    <!-- Paid To -->
                                    <div class="col-md-6 mb-3">
                                        <label for="edit_paidTo" class="form-label">Paid To</label>
                                        <input type="text" class="form-control" id="edit_paidTo" name="paid_to" required>
                                    </div>
                                </div>
                                <div class="row">
                                    <!-- TIN Number -->
                                    <div class="col-md-6 mb-3">
                                        <label for="edit_tinNumber" class="form-label">TIN Number</label>
                                        <input type="text" class="form-control" id="edit_tinNumber" name="tin_number">
                                    </div>
                                    <!-- Date -->
                                    <div class="col-md-6 mb-3">
    ...
    $(document).ready(function() {
        // Function to toggle Cheque Reference No field is removed since it's no longer needed

        // JavaScript for calculating Total is removed since 'Total' is no longer a field
        ...
        $('#addExpenseForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Optionally, refresh the page or append the new expense to the table
                        location.reload();
                    } else {
        ...
        $('.edit-btn').on('click', function() {
            ...
            // Fetch expense data from the table row
            const row = $('#expense-' + expenseId);
            const expenseType = row.find('td:eq(2)').text().trim();
            const paymentMethod = row.find('td:eq(3)').text().trim();
            const cost = row.find('td:eq(4)').text().trim();
            const paidTo = row.find('td:eq(5)').text().trim();
            const tinNumber = row.find('td:eq(6)').text().trim();
            const date = row.find('td:eq(7)').text().trim();
            const description = row.find('td:eq(8)').text().trim();

            // Populate the Edit Modal with fetched data
            $('#edit_expense_id').val(expenseId);
            // Set the expense type by matching the text
            $('#edit_expenseType option').each(function() {
                if ($(this).text().trim() === expenseType) {
                    $(this).prop('selected', true);
                }
            });
            $('#edit_paymentMethod').val(paymentMethod).change();
            $('#edit_cost').val(cost);
            $('#edit_paidTo').val(paidTo);
            $('#edit_tinNumber').val(tinNumber);
            $('#edit_date').val(date);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        const row = $('#expense-' + expenseId);
                        row.find('td:eq(2)').text($('#edit_expenseType option:selected').text());
                        row.find('td:eq(3)').text($('#edit_paymentMethod').val());
                        row.find('td:eq(4)').text($('#edit_cost').val());
                        row.find('td:eq(5)').text($('#edit_paidTo').val());
                        row.find('td:eq(6)').text($('#edit_tinNumber').val());
                        row.find('td:eq(7)').text($('#edit_date').val());
                        row.find('td:eq(8)').text($('#edit_description').val());
