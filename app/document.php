...
// Updated query to fetch documents with search functionality and pagination
$query = "
    SELECT 
        *
    FROM 
...
if ($stmt = $conn->prepare($query)) {
    // Bind parameters: 'sssii' => 3 strings for search, 2 integers for LIMIT
    $stmt->bind_param("sssii", $search_param, $search_param, $search_param, $start_from, $limit);
...
// Fetch category data
$category_sql = "SELECT id, title FROM document_category"; // Corrected table and field names
$category_result = $conn->query($category_sql);
$categories = $category_result->fetch_all(MYSQLI_ASSOC);

// Fetch documents from the database
// $query = "SELECT document_id, title, document FROM document";
...
//         // echo "<div>";
//         // echo "<a href='$documentPath' download='$title'>$title</a>"; // Make the document a downloadable link
//         // echo "</div>";
//     }
// } else {
...
$baseUrl = 'http://localhost/New/Building/html/dashboard/app/uploads/';
$i = $start_from + 1;
        ...
        <div class="sidebar-body pt-0 data-scrollbar">
            <div class="sidebar-list">
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="./document.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="position-relative iq-banner">
        ...
        <nav class="nav navbar navbar-expand-xl navbar-light iq-navbar">
          <div class="container-fluid navbar-inner">
            ...
            <form method="GET" action="document.php" class="input-group search-input">
    <span class="input-group-text" id="search-input">
        <svg class="icon-18" width="18" viewBox="0 0 24 0" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <button type="submit" class="btn btn-primary">Search</button>
     ...
     <div class="container-fluid">
      ...
      <div class="card-header d-flex justify-content-between">
           ...
           <?php if ($role === 'admin'): ?>
           <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addDocumentModal">Add Document</button>
           <?php endif; ?>
           </div>
        <div class="card-body">
    ...
    <?php foreach ($documents as $document): ?>
                    <tr>
                        ...
                        <td data-label="Actions">
                            <?php if ($role === 'admin'): ?>
                            <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editDocumentModal" data-id="<?= htmlspecialchars($document['document_id'], ENT_QUOTES, 'UTF-8') ?>">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-danger" onclick="deleteDocument(<?= htmlspecialchars($document['document_id'], ENT_QUOTES, 'UTF-8') ?>)">
                                <i class="fas fa-trash"></i>
                            </button>
                            <?php endif; ?>
                        </td>
                        <td data-label="User ID"><?= htmlspecialchars($document['user_id'], ENT_QUOTES, 'UTF-8') ?></td>
                        <td data-label="Title"><?= htmlspecialchars($document['title'], ENT_QUOTES, 'UTF-8') ?></td>
                        <td data-label="Category"><?= htmlspecialchars($document['category'], ENT_QUOTES, 'UTF-8') ?></td>
                        <td data-label="Document">
                            <?php
                                if (!empty($document['document'])) {
                                    ...
                                    if ($documentData && is_array($documentData) && isset($documentData[0]['name'])) {
                                        // Extract the file path and normalize it
                                        $documentPath = str_replace(['\\', '//'], '/', $documentData[0]['name']);
                                        $documentTitle = htmlspecialchars($documentData[0]['usrName'] ?? $document['title'], ENT_QUOTES, 'UTF-8');
                                        ...
                                        // Debugging: Output the path being checked
                                        echo "<!-- Checking path: $serverPath -->";

                                        // Check if the file exists on the server
                                        if (file_exists($serverPath)) {
                                            echo "<a href='$fullUrl' download='$documentTitle'>Download</a>";
                                        } else {
                                            echo "File not available";
                                        }
                                    } else {
                                        echo "Invalid document data";
                                    }
                                } else {
                        ...
                        <td data-label="Added By"><?= htmlspecialchars($document['added_by'], ENT_QUOTES, 'UTF-8') ?></td>
                    </tr>
                <?php endforeach; ?>
    </tbody>
</table>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addDocumentForm" enctype="multipart/form-data" method="POST" action="upload_document.php">
                    <!-- User ID -->
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="userId" name="user_id" required>
                            ...
                            <?php foreach ($tenants as $tenant): ?>
                                <option value="<?= htmlspecialchars($tenant['tenant_id']) ?>"><?= htmlspecialchars($tenant['full_name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <!-- Title -->
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="addedBy" name="added_by" required>
                    </div>
                    <!-- Submit Button -->
...
<div class="modal fade" id="editDocumentModal" tabindex="-1" aria-labelledby="editDocumentModalLabel" aria-hidden="true">
            ...
            <div class="modal-body">
                <form id="editDocumentForm">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="edit_userId" name="user_id" required>
                            <option value="">Select Tenant</option>
                            <?php foreach ($tenants as $tenant): ?>
                                <option value="<?= htmlspecialchars($tenant['tenant_id']) ?>"><?= htmlspecialchars($tenant['full_name']) ?></option>
                            <?php endforeach; ?>
                    ...
                    <!-- Added By -->
                    <div class="mb-3">
                        <label for="edit_addedBy" class="form-label">Added By</label>
                        <input type="text" class="form-control" id="edit_addedBy" name="added_by" required>
                    </div>
                    <!-- Submit Button -->
    ...
    .catch(error => {
      ...
      // Handle Edit Document Modal Show Event
      document.getElementById('editDocumentModal').addEventListener('show.bs.modal', function (event) {
        ...
        // Fetch document data and populate the form fields
      ...
      // Function to Fetch Document Data and Populate the Edit Form
      // Function to Fetch Document Data and Populate the Edit Form
function fetchDocumentData(documentId) {
    fetch(`get_document.php?id=${documentId}`)
        ...
        .then(data => {
            if (data) {
                document.getElementById('edit_document_id').value = data.document_id;
                document.getElementById('edit_userId').value = data.user_id;
                document.getElementById('edit_title').value = data.title;
                document.getElementById('edit_category').value = data.category;
                // Do not set the value of the file input
                // document.getElementById('edit_document').value = data.document;
                document.getElementById('edit_addedBy').value = data.added_by;
            } else {
        ...
        .catch(error => {
            ...
            alert('An error occurred while fetching the document data.');
        });
}
