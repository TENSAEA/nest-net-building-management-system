...
$role = isset($_SESSION['role']) ? $_SESSION['role'] : 'user'; // Default to 'user' if role is not set
 ...
 // Fetch floor data from the database with pagination
 $sql = "SELECT * FROM floor LIMIT $start_from, $limit";
  ...
  <body class="  ">
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                    ...
                    <li class="nav-item">
                        ...
                        <ul class="sub-nav collapse" id="utilities-error" data-bs-parent="#sidebar-menu">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" href="./floor.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
      ...
      <div class="position-relative iq-banner">
        ...
        <nav class="nav navbar navbar-expand-xl navbar-light iq-navbar">
          <div class="container-fluid navbar-inner">
           ...
           <h4 class="mt-4">Floor Management</h4>
           <div class="card">
           <div class="card-header d-flex justify-content-between">
                    <h4 class="card-title">Floor</h4>
                    <?php if ($role === 'admin'): ?>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addFloorModal">Add Floor</button>
                    <?php endif; ?>
                    </div>
                    <div class="card-body">
    ...
    <tbody>
        ...
        foreach ($floors as $floor) {
            ...
            if ($role === 'admin') {
                echo "<button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#editFloorModal' data-id='" . htmlspecialchars($floor['floor_id'] ?? '', ENT_QUOTES, 'UTF-8') . "'>
                        <i class='fas fa-edit'></i>
                      </button>
                      <button class='btn btn-sm btn-danger' onclick='deleteFloor(" . htmlspecialchars($floor['floor_id'] ?? '', ENT_QUOTES, 'UTF-8') . ")'>
            ...
            echo "<td data-label='Building ID'>" . htmlspecialchars($floor['building_id'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "<td data-label='Number'>" . htmlspecialchars($floor['number'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "<td data-label='Area'>" . htmlspecialchars($floor['area'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "<td data-label='Price'>" . htmlspecialchars($floor['price'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "<td data-label='Added Date'>" . htmlspecialchars($floor['added_date'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "</tr>";
            $i++;
        }
        ?>
   ...
   <div class="modal fade" id="addFloorModal" tabindex="-1" aria-labelledby="addFloorModalLabel" aria-hidden="true">
          ...
          <div class="modal-body">
            <form id="addFloorForm">
              ...
              <div class="mb-3">
                ...
                <input type="number" class="form-control" id="buildingId" name="building_id" required>
              </div>
              <!-- Number -->
              <div class="mb-3">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="editFloorForm">
              <input type="hidden" id="edit_floor_id" name="floor_id">
              <!-- Building ID -->
              <div class="mb-3">
                <label for="edit_buildingId" class="form-label">Building ID</label>
                <input type="number" class="form-control" id="edit_buildingId" name="building_id" required>
              </div>
              <!-- Number -->
              <div class="mb-3">
    ...
    <script>
       ...
       // Handle Edit Floor Modal Show Event
       document.getElementById('editFloorModal').addEventListener('show.bs.modal', function (event) {
           var button = event.relatedTarget;
           var floorId = button.getAttribute('data-id');
           // Fetch floor data and populate the form fields
           fetchFloorData(floorId);
       });
 
       // Handle Edit Floor Form Submission
       document.getElementById('editFloorForm').addEventListener('submit', function (e) {
           e.preventDefault();
           const formData = new FormData(this);
           fetch('edit_floor.php', {
               method: 'POST',
               body: formData
           })
           .then(response => response.json())
       ...
       // Function to Fetch Floor Data and Populate the Edit Form
       function fetchFloorData(floorId) {
        fetch(`get_floor.php?id=${floorId}`)
          .then(response => response.json())
          .then(data => {
            if (data) {
              document.getElementById('edit_floor_id').value = data.floor_id;
              document.getElementById('edit_buildingId').value = data.building_id;
          ...
          .catch(error => {
            ...
            alert('An error occurred while fetching the floor data.');
          });
      }
       // Handle Delete Floor
       function deleteFloor(floorId) {
           if (confirm('Are you sure you want to delete this floor?')) {
               fetch(`delete_floor.php`, {
                   method: 'DELETE',
                   headers: {
