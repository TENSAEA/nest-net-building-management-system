...
// Fetch document category data from the database with pagination
$sql = "SELECT * FROM document_category LIMIT $start_from, $limit";
$result = $conn->query($sql);
$document_categories = $result->fetch_all(MYSQLI_ASSOC);

// Get total number of records
$sql = "SELECT COUNT(id) FROM document_category";
$result = $conn->query($sql);
  ...
  <body class="  ">
    ...
    <div class="logo-mini">
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                    ...
                    <li class="nav-item">
                        ...
                        <ul class="sub-nav collapse" id="sidebar-auth" data-bs-parent="#sidebar-menu">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" href="./document_cat.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
      ...
      <div class="card-header d-flex justify-content-between">
                    ...
                    <?php if ($role === 'admin'): ?>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addDocumentCategoryModal">Add Document Category</button>
                    <?php endif; ?>
                    </div>
        <div class="card-body">
    ...
    <tbody>
        ...
        foreach ($document_categories as $document_category) {
            ...
            if ($role === 'admin') {
                echo "<button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#editDocumentCategoryModal' data-id='" . htmlspecialchars($document_category['id'] ?? '', ENT_QUOTES, 'UTF-8') . "'>
                        <i class='fas fa-edit'></i>
                      </button>
                      <button class='btn btn-sm btn-danger' onclick='deleteDocumentCategory(" . htmlspecialchars($document_category['id'] ?? '', ENT_QUOTES, 'UTF-8') . ")'>
            ...
            echo "<td data-label='Title'>" . htmlspecialchars($document_category['title'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "</tr>";
            $i++;
        }
        ?>
    ...
    <div class="modal fade" id="addDocumentCategoryModal" tabindex="-1" aria-labelledby="addDocumentCategoryModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="addDocumentCategoryModalLabel">Add Document Category</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form id="addDocumentCategoryForm">
              <!-- Title -->
              <div class="mb-3">
    ...
    <div class="modal fade" id="editDocumentCategoryModal" tabindex="-1" aria-labelledby="editDocumentCategoryModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editDocumentCategoryModalLabel">Edit Document Category</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form id="editDocumentCategoryForm">
              <input type="hidden" id="edit_document_category_id" name="id">
              <!-- Title -->
              <div class="mb-3">
              ...
              <!-- Submit Button -->
              <button type="submit" class="btn btn-primary">Update Document Category</button>
            </form>
          </div>
        </div>
      </div>
    ...
    <script>
      // Handle Add Document Category Form Submission
      document.getElementById('addDocumentCategoryForm').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);
        fetch('add_document_category.php', {
          method: 'POST',
          body: formData
        })
        ...
        .then(data => {
          if (data.success) {
            ...
            alert('Failed to add document category. Please try again.');
            console.error('Error:', data.error);
          }
        })
        .catch(error => {
          console.error('Error adding document category:', error);
          alert('An error occurred while adding the document category.');
        });
      });

      // Handle Edit Document Category Modal Show Event
      document.getElementById('editDocumentCategoryModal').addEventListener('show.bs.modal', function (event) {
        var button = event.relatedTarget;
        var documentCategoryId = button.getAttribute('data-id');
        // Fetch document category data and populate the form fields
        fetchDocumentCategoryData(documentCategoryId);
      });

      // Handle Edit Document Category Form Submission
      document.getElementById('editDocumentCategoryForm').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);
        fetch('edit_document_category.php', {
          method: 'POST',
          body: formData
        })
        .then(response => response.json())
        .then(data => {
          if (data.success) {
            ...
            alert('Failed to update document category. Please try again.');
          }
        })
        .catch(error => {
          console.error('Error updating document category:', error);
          alert('An error occurred while updating the document category.');
        });
      });

      // Function to Fetch Document Category Data and Populate the Edit Form
      function fetchDocumentCategoryData(documentCategoryId) {
        fetch(`get_document_category.php?id=${documentCategoryId}`)
          .then(response => response.json())
          .then(data => {
            if (data) {
              document.getElementById('edit_document_category_id').value = data.id;
              document.getElementById('edit_title').value = data.title;
              document.getElementById('edit_order').value = data.order;
            } else {
              alert('Failed to fetch document category data.');
            }
          })
          .catch(error => {
            console.error('Error fetching document category data:', error);
            alert('An error occurred while fetching the document category data.');
          });
      }

      // Handle Delete Document Category
      function deleteDocumentCategory(documentCategoryId) {
        if (confirm('Are you sure you want to delete this document category?')) {
          fetch(`delete_document_category.php`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded',
            },
            body: `id=${documentCategoryId}`
          })
          .then(response => response.json())
          .then(data => {
            if (data.success) {
              alert('Document Category deleted successfully.');
              location.reload();
            } else {
              alert('Failed to delete document category.');
              console.error('Error:', data.error);
            }
          })
          .catch(error => {
            console.error('Error deleting document category:', error);
            alert('An error occurred while deleting the document category.');
          });
        }
      }
    </script>
