...
$role = isset($_SESSION['role']) ? $_SESSION['role'] : 'user'; // Default to 'user' if role is not set
 ...
 // Fetch penality data from the database with pagination
 $sql = "SELECT * FROM penality LIMIT $start_from, $limit";
 $result = $conn->query($sql);
 $penalities = $result->fetch_all(MYSQLI_ASSOC);
 
 // Get total number of records
 $sql = "SELECT COUNT(id) FROM penality";
 $result = $conn->query($sql);
  ...
  <body class="  ">
      ...
      <div class="position-relative iq-banner">
        ...
        <nav class="nav navbar navbar-expand-xl navbar-light iq-navbar">
          <div class="container-fluid navbar-inner">
           ...
           <h4 class="mt-4">Penality Management</h4>
           <?php if ($role === 'admin'): ?>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addPenalityModal">Add Penality</button>
                    <?php endif; ?>
                    </div>
               <div class="card-body">
    ...
    <tbody>
        ...
        foreach ($penalities as $penality) {
            ...
            if ($role === 'admin') {
                echo "<button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#editPenalityModal' data-id='" . htmlspecialchars($penality['id'] ?? '', ENT_QUOTES, 'UTF-8') . "'>
                        <i class='fas fa-edit'></i>
                      </button>
                      <button class='btn btn-sm btn-danger' onclick='deletePenality(" . htmlspecialchars($penality['id'] ?? '', ENT_QUOTES, 'UTF-8') . ")'>
            ...
            echo "<td data-label='Percent'>" . htmlspecialchars($penality['percent'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "<td data-label='Measure'>" . htmlspecialchars($penality['measure'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "<td data-label='Color'>" . htmlspecialchars($penality['color'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "</tr>";
            $i++;
        }
        ?>
...
<!-- Add Penality Modal -->
<div class="modal fade" id="addPenalityModal" tabindex="-1" aria-labelledby="addPenalityModalLabel" aria-hidden="true">
           <div class="modal-dialog modal-lg">
               <div class="modal-content">
                   <div class="modal-header">
                       <h5 class="modal-title" id="addPenalityModalLabel">Add Penality</h5>
                       <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                   </div>
                   <div class="modal-body">
                       <form id="addPenalityForm">
                           <!-- Percent -->
                           <div class="mb-3">
                           ...
                           <!-- Submit Button -->
                           <button type="submit" class="btn btn-primary">Add Penality</button>
                       </form>
                   </div>
               </div>
           </div>
     ...
     <!-- Edit Penality Modal -->
     <div class="modal fade" id="editPenalityModal" tabindex="-1" aria-labelledby="editPenalityModalLabel" aria-hidden="true">
           <div class="modal-dialog modal-lg">
               <div class="modal-content">
                   <div class="modal-header">
                       <h5 class="modal-title" id="editPenalityModalLabel">Edit Penality</h5>
                       <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                   </div>
                   <div class="modal-body">
                       <form id="editPenalityForm">
                           <input type="hidden" id="edit_penality_id" name="id">
                           <!-- Percent -->
                           <div class="mb-3">
                               <label for="edit_percent" class="form-label">Percent</label>
                               <input type="text" class="form-control" id="edit_percent" name="percent" required>
                           </div>
                           <!-- Measure -->
                           <div class="mb-3">
                               <label for="edit_measure" class="form-label">Measure</label>
                               <input type="text" class="form-control" id="edit_measure" name="measure" required>
                           </div>
                           <!-- Color -->
                           <div class="mb-3">
                           ...
                           <!-- Submit Button -->
                           <button type="submit" class="btn btn-primary">Update Penality</button>
                       </form>
                   </div>
               </div>
           </div>
    ...
    <script>
       // Handle Add Penality Form Submission
       document.getElementById('addPenalityForm').addEventListener('submit', function (e) {
           e.preventDefault();
           const formData = new FormData(this);
           fetch('add_penality.php', {
               method: 'POST',
               body: formData
           })
           ...
           .then(data => {
               if (data.success) {
                   alert('Penality added successfully.');
                   location.reload();
               } else {
                   alert('Failed to add penality. Please try again.');
                   console.error('Error:', data.error);
               }
           })
           .catch(error => {
               console.error('Error adding penality:', error);
               alert('An error occurred while adding the penality.');
           });
       });
 
       // Handle Edit Penality Modal Show Event
       document.getElementById('editPenalityModal').addEventListener('show.bs.modal', function (event) {
           var button = event.relatedTarget;
           var penalityId = button.getAttribute('data-id');
           // Fetch penality data and populate the form fields
           fetchPenalityData(penalityId);
       });
 
       // Handle Edit Penality Form Submission
       document.getElementById('editPenalityForm').addEventListener('submit', function (e) {
           e.preventDefault();
           const formData = new FormData(this);
           fetch('edit_penality.php', {
               method: 'POST',
               body: formData
           })
           .then(response => response.json())
           .then(data => {
               if (data.success) {
                   alert('Penality updated successfully.');
                   location.reload();
               } else {
                   alert('Failed to update penality. Please try again.');
               }
           })
           .catch(error => {
               console.error('Error updating penality:', error);
               alert('An error occurred while updating the penality.');
           });
       });
 
       // Function to Fetch Penality Data and Populate the Edit Form
       function fetchPenalityData(penalityId) {
           fetch(`get_penality.php?id=${penalityId}`)
               .then(response => response.json())
               .then(data => {
                   if (data) {
                       document.getElementById('edit_penality_id').value = data.id;
                       document.getElementById('edit_percent').value = data.percent;
                       document.getElementById('edit_measure').value = data.measure;
                       document.getElementById('edit_color').value = data.color;
                   } else {
                       alert('Failed to fetch penality data.');
                   }
               })
               .catch(error => {
                   console.error('Error fetching penality data:', error);
                   alert('An error occurred while fetching the penality data.');
               });
       }
 
       // Handle Delete Penality
       function deletePenality(penalityId) {
           if (confirm('Are you sure you want to delete this penality?')) {
               fetch(`delete_penality.php`, {
                   method: 'DELETE',
                   headers: {
               ...
               .then(data => {
                   if (data.success) {
                       alert('Penality deleted successfully.');
                       location.reload();
                   } else {
                       alert('Failed to delete penality.');
                       console.error('Error:', data.error);
                   }
               })
               .catch(error => {
                   console.error('Error deleting penality:', error);
                   alert('An error occurred while deleting the penality.');
               });
           }
       }
       </script>
