...
// Fetch tenant property data from the database with pagination
$sql = "SELECT tp.*, t.full_name as tenant_full_name, t.room as tenant_room 
...
$stmt->execute();
$result = $stmt->get_result();
$tenant_properties = $result->fetch_all(MYSQLI_ASSOC);

// Get total number of records
$sql = "SELECT COUNT(id) FROM tenants_property";
...
// Fetch tenants
$tenant_query = "SELECT tenant_id, full_name FROM tenant ORDER BY full_name ASC";
...
if ($tenant_result) {
    ...
    // Handle query error
    echo "Error fetching tenants: " . mysqli_error($conn);
}
?>
...
@media (max-width: 768px) {
  ...
  <body class="  ">
    ...
    <div class="logo-mini">
        ...
        <div class="sidebar-body pt-0 data-scrollbar">
            <div class="sidebar-list">
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="./tenant_property.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="card-header d-flex justify-content-between">
           <h4 class="mt-4">Tenant Property</h4>
           <?php if ($role === 'admin'): ?>

           <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addTenantPropertyModal">Add Tenant Property</button>
           <?php endif; ?>
                    </div>
        <div class="card-body">
          ...
          <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th>Tenant</th>
                        <th>Room</th>
                        <th>Tenants Property No</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $i = $start_from + 1;
                    foreach ($tenant_properties as $tenants_property) {
                        echo "<tr>";
                        echo "<td data-label='No'>{$i}</td>"; // Incremental number
                        echo "<td data-label='Actions'>";

                        if ($role === 'admin') {
                            echo "<button class='btn btn-sm btn-primary edit-btn' data-bs-toggle='modal' data-bs-target='#editTenantPropertyModal' data-id='" . htmlspecialchars($tenants_property['id'], ENT_QUOTES, 'UTF-8') . "'>
                                    <i class='fas fa-edit'></i>
                                  </button>
                                  <button class='btn btn-sm btn-danger' onclick='deleteTenantProperty(" . htmlspecialchars($tenants_property['id'], ENT_QUOTES, 'UTF-8') . ")'>
                        ...
                        echo "<td data-label='Tenant'>" . htmlspecialchars($tenants_property['tenant_full_name'], ENT_QUOTES, 'UTF-8') . "</td>";
                        echo "<td data-label='Room'>" . htmlspecialchars($tenants_property['tenant_room'], ENT_QUOTES, 'UTF-8') . "</td>";
                        echo "<td data-label='Tenants Property No'>" . htmlspecialchars($tenants_property['tenants_property_no'], ENT_QUOTES, 'UTF-8') . "</td>";
                        echo "</tr>";
                        $i++;
                    }
                    ?>
 ...
 <!-- Add Tenant Property Modal -->
 <div class="modal fade" id="addTenantPropertyModal" tabindex="-1" aria-labelledby="addTenantPropertyModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add Tenant Property</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="addTenantPropertyForm">
                        <div class="mb-3">
                            <label for="tenant_select" class="form-label">Tenant</label>
                            <select class="form-select" id="tenant_select" name="tenant_id" required>
                                <option value="">Select Tenant</option>
                                <?php foreach ($tenants as $tenant): ?>
                                    <option value="<?= htmlspecialchars($tenant['tenant_id'], ENT_QUOTES, 'UTF-8') ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="room" name="room" readonly required>
                        </div>
                        <div class="mb-3">
                            <label for="tenants_property_no" class="form-label">Tenants Property No</label>
                            <input type="text" class="form-control" id="tenants_property_no" name="tenants_property_no" value="<?= uniqid() ?>" readonly>
                        </div>
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5 class="card-title">Tenants Property Details</h5>
                                <button type="button" class="btn btn-primary" id="addPropertyRow">Inline Add</button>
                            </div>
                            <div class="card-body">
                                <table class="table table-bordered" id="propertyTable">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td><input type="text" class="form-control" name="property_name[]" required></td>
    ...
    <!-- Edit Tenant Property Modal -->
    <div class="modal fade" id="editTenantPropertyModal" tabindex="-1" aria-labelledby="editTenantPropertyModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Edit Tenant Property</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editTenantPropertyForm">
                        <input type="hidden" id="edit_id" name="id">
                        <div class="mb-3">
                            <label for="edit_tenant_select" class="form-label">Tenant</label>
                            <select class="form-select" id="edit_tenant_select" name="tenant_id" required>
                                <option value="">Select Tenant</option>
                                <?php foreach ($tenants as $tenant): ?>
                                    <option value="<?= htmlspecialchars($tenant['tenant_id'], ENT_QUOTES, 'UTF-8') ?>">
                                        <?= htmlspecialchars($tenant['full_name'], ENT_QUOTES, 'UTF-8') ?>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="edit_room" name="room" readonly required>
                        </div>
                        <div class="mb-3">
                            <label for="edit_tenants_property_no" class="form-label">Tenants Property No</label>
                            <input type="text" class="form-control" id="edit_tenants_property_no" name="tenants_property_no" readonly>
                        </div>
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5 class="card-title">Tenants Property Details</h5>
                                <button type="button" class="btn btn-primary" id="editAddPropertyRow">Inline Add</button>
                            </div>
                            <div class="card-body">
                                <table class="table table-bordered" id="editPropertyTable">
                                    <thead>
                                        <tr>
                                            <th>Property Name</th>
                                            <th>Quantity</th>
    ...
    <script>
        // Function to fetch room based on tenant ID
        function fetchRoom(tenantId, roomInputId) {
            if (tenantId === '') {
                document.getElementById(roomInputId).value = '';
                return;
            }

            console.log(`Fetching room for tenant ID: ${tenantId}`); // Debug log

            fetch(`get_tenant_room.php?tenant_id=${tenantId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        document.getElementById(roomInputId).value = data.room;
                    } else {
        ...
        // Event listener for Add Tenant Property Modal
        document.getElementById('tenant_select').addEventListener('change', function() {
            const tenantId = this.value;
            console.log(`Selected tenant ID: ${tenantId}`); // Debug log
            fetchRoom(tenantId, 'room');
        });

        // Add Property Row in Add Modal
        ...
        // Remove Property Row in Add Modal
        document.getElementById('propertyTable').addEventListener('click', function(e) {
        ...
        // Add Tenant Property Form Submission
        document.getElementById('addTenantPropertyForm').addEventListener('submit', function(e) {
            e.preventDefault();
            ...
            fetch('add_tenant_property_details.php', {  // Ensure the correct endpoint
                method: 'POST',
                body: formData
            })
            .then(response => {
            ...
            .then(data => {
                ...
                if (data.success) {
                    alert('Tenant property added successfully.');
                    location.reload();
                } else {
                    alert('Failed to add tenant property. Error: ' + (data.error || 'Unknown error'));
                    console.error('Error:', data.error);
                }
            })
            .catch(error => {
                console.error('Error adding tenant property:', error);
                alert('An error occurred while adding the tenant property: ' + error.message);
            });
        });

        // Edit Tenant Property Modal Show Event
        document.getElementById('editTenantPropertyModal').addEventListener('show.bs.modal', function(event) {
            const button = event.relatedTarget;
            const id = button.getAttribute('data-id');
            fetchTenantPropertyData(id);
        });

        // Function to fetch tenant property data and populate the edit form
        function fetchTenantPropertyData(id) {
            fetch(`get_tenant_property.php?id=${id}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    ...
                    if (data && !data.error) {
                        ...
                        document.getElementById('edit_tenant_select').value = data.tenant_id;
                        document.getElementById('edit_room').value = data.room;
                        document.getElementById('edit_tenants_property_no').value = data.tenants_property_no;

                        // Clear existing rows
                        const tableBody = document.getElementById('editPropertyTable').getElementsByTagName('tbody')[0];
                        tableBody.innerHTML = '';

                        // Add property details rows
                        if (data.details && Array.isArray(data.details)) {
                            data.details.forEach(detail => {
                                const newRow = tableBody.insertRow();
                                newRow.innerHTML = `
                                    <td><input type="text" class="form-control" name="property_name[]" value="${detail.property_name}" required></td>
                                    <td><input type="number" class="form-control" name="quantity[]" value="${detail.quantity}" required></td>
                                    <td><input type="text" class="form-control" name="description[]" value="${detail.description}" required></td>
                                    <td><button type="button" class="btn btn-danger removeRow">Remove</button></td>
                                `;
                            });
                        } else {
                            console.error('No details found or details is not an array');
                        }

                        // Trigger change event to populate the room field
                        fetchRoom(data.tenant_id, 'edit_room');
                    } else {
                        alert('Failed to fetch tenant property data: ' + (data.error || 'Unknown error'));
                    }
                })
                .catch(error => {
                    console.error('Error fetching tenant property data:', error);
                    alert('An error occurred while fetching the tenant property data: ' + error.message);
                });
        }

        // Add Property Row in Edit Modal
        document.getElementById('editAddPropertyRow').addEventListener('click', function() {
            const table = document.getElementById('editPropertyTable').getElementsByTagName('tbody')[0];
            const newRow = table.insertRow();
            newRow.innerHTML = `
                <td><input type="text" class="form-control" name="property_name[]" required></td>
                <td><input type="number" class="form-control" name="quantity[]" required></td>
        ...
        // Remove Property Row in Edit Modal
        document.getElementById('editPropertyTable').addEventListener('click', function(e) {
            if (e.target && e.target.classList.contains('removeRow')) {
                const row = e.target.closest('tr');
                row.parentNode.removeChild(row);
            }
        ...
        // Edit Tenant Property Form Submission
        document.getElementById('editTenantPropertyForm').addEventListener('submit', function(e) {
            e.preventDefault();
            const formData = new FormData(this);
            ...
            fetch('edit_tenant_property_details.php', {
                method: 'POST',
                body: formData
            })
            .then(response => {
            ...
            .then(data => {
                console.log('Parsed data:', data);
                if (data.success) {
                    alert('Tenant property updated successfully.');
                    location.reload();
                } else {
                    alert('Failed to update tenant property. Error: ' + (data.error || 'Unknown error'));
                    console.error('Error:', data.error);
                }
            })
            .catch(error => {
                console.error('Error updating tenant property:', error);
                alert('An error occurred while updating the tenant property: ' + error.message);
            });
        });

        // Handle Delete Tenant Property
        function deleteTenantProperty(id) {
            if (confirm('Are you sure you want to delete this tenant property?')) {
                fetch(`delete_tenants_property.php`, {
                    method: 'DELETE',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        alert('Tenant property deleted successfully.');
                        location.reload();
                    } else {
                        alert('Failed to delete tenant property.');
                        console.error('Error:', data.error);
                    }
                })
                .catch(error => {
                    console.error('Error deleting tenant property:', error);
                    alert('An error occurred while deleting the tenant property.');
                });
            }
        }

        // Event listener for Edit Tenant Property Modal's tenant select
        document.getElementById('edit_tenant_select').addEventListener('change', function() {
            const tenantId = this.value;
            console.log(`Selected tenant ID (edit): ${tenantId}`); // Debug log
            fetchRoom(tenantId, 'edit_room');
        });
    </script>
  </body>
</html>