...
//  $sql = "SELECT * FROM room LIMIT $start_from, $limit";
...
// Updated query to fetch rooms with search functionality and pagination
$query = "
    SELECT 
        *
    FROM 
    ...
    WHERE 
        ...
        category LIKE ? OR
        area LIKE ? OR
        sqt_mtr_price LIKE ? OR
        monthly_price LIKE ? OR
        tenant LIKE ? OR
        status LIKE ? OR
        added_by LIKE ?
    ORDER BY 
...
if ($stmt = $conn->prepare($query)) {
    // Bind parameters: 'ssssssssssi' => 10 strings for search, 2 integers for LIMIT
    $stmt->bind_param("sssssssssssi", $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $start_from, $limit);
 ...
 $sql = "SELECT COUNT(room_id) FROM room";
 $result = $conn->query($sql);
...
@media (max-width: 768px) {
  ...
  <body class="  ">
      ...
      <div class="position-relative iq-banner">
        ...
        <nav class="nav navbar navbar-expand-xl navbar-light iq-navbar">
          <div class="container-fluid navbar-inner">
            ...
            <form method="GET" action="room.php" class="input-group search-input">
        ...
        <div class="container-fluid">
           ...
           <!-- <h4 class="mt-4">Room Management</h4> -->
    ...
    foreach ($rooms as $room) {
        ...
        if ($role === 'admin') {
            echo "<button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#editRoomModal' data-id='" . htmlspecialchars($room['room_id'] ?? '', ENT_QUOTES, 'UTF-8') . "'>
                    <i class='fas fa-edit'></i>
                  </button>
                  <button class='btn btn-sm btn-danger' onclick='deleteRoom(" . htmlspecialchars($room['room_id'] ?? '', ENT_QUOTES, 'UTF-8') . ")'>
        ...
        echo "<td data-label='Building'>" . htmlspecialchars($room['building'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Floor'>" . htmlspecialchars($room['floor'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Room No'>" . htmlspecialchars($room['room_no'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Category'>" . htmlspecialchars($room['category'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Area'>" . htmlspecialchars($room['area'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Price per Square Meter'>" . htmlspecialchars($room['sqt_mtr_price'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Monthly Price'>" . htmlspecialchars($room['monthly_price'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Tenant'>" . htmlspecialchars($room['tenant'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Status'>" . htmlspecialchars($room['status'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "<td data-label='Added By'>" . htmlspecialchars($room['added_by'] ?? 'N/A', ENT_QUOTES, 'UTF-8') . "</td>";
        echo "</tr>";
        $i++;
    }
    ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addRoomForm">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="building" name="building" required>
                            ...
                            <?php foreach ($buildings as $building): ?>
                                <option value="<?= htmlspecialchars($building['building_id'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($building['building_name'], ENT_QUOTES, 'UTF-8') ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                                        <!-- Floor -->
                        ...
                        <select class="form-select" id="floor" name="floor" required>
                            ...
                            <?php foreach ($floors as $floor): ?>
                                <option value="<?= htmlspecialchars($floor['floor_id'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($floor['number'], ENT_QUOTES, 'UTF-8') ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <!-- Room No -->
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="category" name="category" required>
                            ...
                            <?php foreach ($all_categories as $cat): ?>
                                <option value="<?= htmlspecialchars($cat['id'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($cat['title'], ENT_QUOTES, 'UTF-8') ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <!-- Area -->
                    ...
                    <!-- Square Meter Price -->
                    <div class="mb-3">
                        <label for="sqt_mtr_price" class="form-label">Price per Square Meter</label>
                        <input type="number" step="0.01" class="form-control" id="sqt_mtr_price" name="sqt_mtr_price" required>
                    </div>
                    <!-- Monthly Price -->
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="status" name="status"  value="Vacant" required readonly>
                    </div>
                    <!-- Added By -->
                    <div class="mb-3">
                        <label for="added_by" class="form-label">Added By</label>
                        <input type="text" class="form-control" id="added_by" name="added_by" required readonly value="<?php echo $_SESSION['username']; ?>">
                    </div>
                    <!-- Submit Button -->
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editRoomForm">
                    ...
                    <!-- Room No -->
                    <div class="mb-3">
                        <label for="edit_room_no" class="form-label">Room No</label>
                        <input type="text" class="form-control" id="edit_room_no" name="room_no" required>
                    </div>
                    <!-- Category -->
                    <div class="mb-3">
                    ...
                    <!-- Area -->
                    <div class="mb-3">
                        <label for="edit_area" class="form-label">Area (sq meters)</label>
                        <input type="number" step="0.01" class="form-control" id="edit_area" name="area" required>
                    </div>
                    <!-- Square Meter Price -->
                    <div class="mb-3">
                        <label for="edit_sqt_mtr_price" class="form-label">Price per Square Meter</label>
                        <input type="number" step="0.01" class="form-control" id="edit_sqt_mtr_price" name="sqt_mtr_price" required>
                    </div>
                    <!-- Monthly Price -->
                    <div class="mb-3">
                        <label for="edit_monthly_price" class="form-label">Monthly Price</label>
                        <input type="number" step="0.01" class="form-control" id="edit_monthly_price" name="monthly_price" required>
                    </div>
                    <!-- Tenant -->
                    <div class="mb-3">
                        <label for="edit_tenant" class="form-label">Tenant</label>
                        <input type="text" class="form-control" id="edit_tenant" name="tenant" required>
                    </div>
                    <!-- Status -->
                    <div class="mb-3">
                    ...
                    <!-- Added By -->
                    <div class="mb-3">
                        <label for="edit_added_by" class="form-label">Added By</label>
                        <input type="text" class="form-control" id="edit_added_by" name="added_by" required>
                    </div>
                    <!-- Submit Button -->
...
document.getElementById('addRoomForm').addEventListener('submit', function (e) {
    ...
    .then(response => {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            throw new TypeError('Expected JSON response but received different content.');
        }
...
<script>
    ...
    // Handle Edit Room Modal Show Event
    ...
    editRoomModal.addEventListener('show.bs.modal', function (event) {
        ...
        // Fetch room data and populate the form fields
        fetch(`get_room.php?id=${roomId}`)
            ...
            .then(data => {
                if (data) {
                    ...
                    document.getElementById('edit_sqt_mtr_price').value = data.sqt_mtr_price;
                    document.getElementById('edit_monthly_price').value = data.monthly_price;
                    document.getElementById('edit_tenant').value = data.tenant;
                    document.getElementById('edit_status').value = data.status;
                    document.getElementById('edit_added_by').value = data.added_by;
                } else {
            ...
            .catch(error => {
                ...
                alert('An error occurred while fetching the room data.');
            });
    });
