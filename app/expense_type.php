...
// Fetch expense type data from the database with pagination
$sql = "SELECT * FROM expense_type LIMIT $start_from, $limit";
$result = $conn->query($sql);
$expense_types = $result->fetch_all(MYSQLI_ASSOC);

// Get total number of records
$sql = "SELECT COUNT(id) FROM expense_type";
$result = $conn->query($sql);
  ...
  <body class="  ">
    ...
    <div class="logo-mini">
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                    ...
                    <li class="nav-item">
                        ...
                        <ul class="sub-nav collapse" id="sidebar-auth" data-bs-parent="#sidebar-menu">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" href="./expense_type.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
      ...
      <div class="card-header d-flex justify-content-between">
                    ...
                    <?php if ($role === 'admin'): ?>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addExpenseTypeModal">Add Expense Type</button>
                    <?php endif; ?>
                    </div>
        <div class="card-body">
    ...
    <tbody>
        ...
        foreach ($expense_types as $expense_type) {
            ...
            if ($role === 'admin') {
                echo "<button class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#editExpenseTypeModal' data-id='" . htmlspecialchars($expense_type['id'] ?? '', ENT_QUOTES, 'UTF-8') . "'>
                        <i class='fas fa-edit'></i>
                      </button>
                      <button class='btn btn-sm btn-danger' onclick='deleteExpenseType(" . htmlspecialchars($expense_type['id'] ?? '', ENT_QUOTES, 'UTF-8') . ")'>
            ...
            echo "<td data-label='Title'>" . htmlspecialchars($expense_type['title'] ?? '', ENT_QUOTES, 'UTF-8') . "</td>";
            echo "</tr>";
            $i++;
        }
        ?>
   ...
   <div class="modal fade" id="addExpenseTypeModal" tabindex="-1" aria-labelledby="addExpenseTypeModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="editExpenseTypeForm">
              <input type="hidden" id="edit_expense_type_id" name="id">
              <!-- Title -->
              <div class="mb-3">
    ...
    <script>
      // Handle Add Expense Type Form Submission
      document.getElementById('addExpenseTypeForm').addEventListener('submit', function (e) {
        ...
        .catch(error => {
          ...
          alert('An error occurred while adding the expense type.');
        });
      });

      // Handle Edit Expense Type Modal Show Event
      document.getElementById('editExpenseTypeModal').addEventListener('show.bs.modal', function (event) {
        var button = event.relatedTarget;
        var expenseTypeId = button.getAttribute('data-id');
        // Fetch expense type data and populate the form fields
        fetchExpenseTypeData(expenseTypeId);
      });

      // Handle Edit Expense Type Form Submission
      document.getElementById('editExpenseTypeForm').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);
        fetch('edit_expense_type.php', {
          method: 'POST',
          body: formData
        })
        .then(response => response.json())
        .then(data => {
          if (data.success) {
            ...
            alert('Failed to update expense type. Please try again.');
          }
        })
        .catch(error => {
          console.error('Error updating expense type:', error);
          alert('An error occurred while updating the expense type.');
        });
      });

      // Function to Fetch Expense Type Data and Populate the Edit Form
      function fetchExpenseTypeData(expenseTypeId) {
        fetch(`get_expense_type.php?id=${expenseTypeId}`)
          .then(response => response.json())
          .then(data => {
            if (data) {
              document.getElementById('edit_expense_type_id').value = data.id;
          ...
          .catch(error => {
            ...
            alert('An error occurred while fetching the expense type data.');
          });
      }
      ...
      function deleteExpenseType(expenseTypeId) {
        if (confirm('Are you sure you want to delete this expense type?')) {
          fetch(`delete_expense_type.php`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded',
            },
            body: `id=${expenseTypeId}`
          })
          .then(response => response.json())
          .then(data => {
            if (data.success) {
