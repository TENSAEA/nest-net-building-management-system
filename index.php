...
include './db.php';
// Fetch all notifications for server-side rendering
$notifications = [];
$notificationsStmt = $conn->prepare("SELECT id, message, status, created_at FROM notifications ORDER BY created_at DESC LIMIT 10");
if ($notificationsStmt) {
    if ($notificationsStmt->execute()) {
        $notificationsResult = $notificationsStmt->get_result();
        $notifications = $notificationsResult->fetch_all(MYSQLI_ASSOC);
    } else {
        error_log("Execute failed: " . $notificationsStmt->error);
    }
...
// 1. Fetch Number of Tenants
$tenant_count_sql = "SELECT COUNT(*) AS tenant_count FROM tenant";
$tenant_count_result = $conn->query($tenant_count_sql);
$tenant_count = $tenant_count_result->fetch_assoc()['tenant_count'] ?? 0;

// 2. Fetch This Month's Income
$this_month_income_sql = "
    SELECT SUM(total) AS income
    FROM payment
    WHERE MONTH(payment_date) = MONTH(CURRENT_DATE())
      AND YEAR(payment_date) = YEAR(CURRENT_DATE())
";
$this_month_income_result = $conn->query($this_month_income_sql);
$this_month_income = $this_month_income_result->fetch_assoc()['income'] ?? 0;

// 3. Fetch This Month's Expense
$this_month_expense_sql = "
    SELECT SUM(cost) AS expense
    FROM expense
    WHERE MONTH(date) = MONTH(CURRENT_DATE())
      AND YEAR(date) = YEAR(CURRENT_DATE())
";
$this_month_expense_result = $conn->query($this_month_expense_sql);
$this_month_expense = $this_month_expense_result->fetch_assoc()['expense'] ?? 0;

// 4. Fetch This Week's Receivables
// Calculate start and end of the current week (Monday to Sunday)
$start_of_week = date('Y-m-d', strtotime('monday this week'));
$end_of_week = date('Y-m-d', strtotime('sunday this week'));

// Alternatively, if your week starts on Sunday:
// $start_of_week = date('Y-m-d', strtotime('sunday last week'));
// $end_of_week = date('Y-m-d', strtotime('saturday this week'));

$this_week_receivables_sql = "
    SELECT SUM(rent_amount) AS receivables
    FROM tenant
    WHERE next_payment_date >= ? AND next_payment_date <= ?
";
$stmt = $conn->prepare($this_week_receivables_sql);
if ($stmt) {
    $stmt->bind_param("ss", $start_of_week, $end_of_week);
    $stmt->execute();
    $this_week_receivables_result = $stmt->get_result();
    $this_week_receivables = $this_week_receivables_result->fetch_assoc()['receivables'] ?? 0;
    $stmt->close();
} else {
    // Handle statement preparation error
    error_log("Error preparing statement: " . $conn->error);
    $this_week_receivables = 0;
}
 // Fetch Tenant Growth Over Time
$tenant_growth_sql = "
SELECT 
...
ORDER BY YEAR(added_date), MONTH(added_date)
";
$tenant_growth_result = $conn->query($tenant_growth_sql);

$tenant_growth = [];
$tenant_months = [];
if ($tenant_growth_result) {
while ($row = $tenant_growth_result->fetch_assoc()) {
    $month_year = date("M Y", strtotime("{$row['year']}-{$row['month']}-01"));
    $tenant_months[] = $month_year;
    $tenant_growth[] = (int)$row['tenant_count'];
}
} else {
error_log("Error fetching tenant growth: " . $conn->error);
}

// Fetch Monthly Payments Received
$monthly_payments_sql = "
SELECT 
    YEAR(payment_date) AS year, 
    MONTH(payment_date) AS month, 
    SUM(total) AS payments_received
FROM payment
...
while ($row = $monthly_payments_result->fetch_assoc()) {
    $month_year = date("M Y", strtotime("{$row['year']}-{$row['month']}-01"));
    $payment_months[] = $month_year;
    $monthly_payments[] = (float)$row['payments_received'];
}
} else {
error_log("Error fetching monthly payments: " . $conn->error);
}

// Fetch Revenue vs. Expenses
$revenue_expenses_sql = "
SELECT 
    YEAR(payment_date) AS year, 
    MONTH(payment_date) AS month, 
    SUM(payment.total) AS revenue,
    SUM(expense.cost) AS expenses
FROM 
    payment
LEFT JOIN 
    expense ON YEAR(payment_date) = YEAR(expense.date) AND MONTH(payment_date) = MONTH(expense.date)
GROUP BY 
    YEAR(payment_date), MONTH(payment_date)
ORDER BY 
    YEAR(payment_date), MONTH(payment_date)
";
$revenue_expenses_result = $conn->query($revenue_expenses_sql);

$revenue = [];
$expenses = [];
$revenue_expense_months = [];
if ($revenue_expenses_result) {
while ($row = $revenue_expenses_result->fetch_assoc()) {
    $month_year = date("M Y", strtotime("{$row['year']}-{$row['month']}-01"));
    $revenue_expense_months[] = $month_year;
    $revenue[] = (float)$row['revenue'];
    $expenses[] = (float)$row['expenses'];
}
} else {
error_log("Error fetching revenue and expenses: " . $conn->error);
}
  ...
  <body class="  ">
    ...
    <div class="logo-normal">
        <svg class="icon-30" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <rect x="16" y="16" width="8" height="8" fill="#FFEB3B"/>
            <rect x="28" y="16" width="8" height="8" fill="#FFEB3B"/>
            <rect x="40" y="16" width="8" height="8" fill="#FFEB3B"/>
            <rect x="16" y="28" width="8" height="8" fill="#FFEB3B"/>
            <rect x="28" y="28" width="8" height="8" fill="#FFEB3B"/>
            <rect x="40" y="28" width="8" height="8" fill="#FFEB3B"/>
            <rect x="16" y="40" width="8" height="8" fill="#FFEB3B"/>
            <rect x="28" y="40" width="8" height="8" fill="#FFEB3B"/>
            <rect x="40" y="40" width="8" height="8" fill="#FFEB3B"/>
            <rect x="28" y="48" width="8" height="8" fill="#FF5722"/>
        </svg>
    </div>
    <div class="logo-mini">
        <svg class="icon-30" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div class="logo-main">
            ...
            <div class="sidebar-list">
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                    ...
                    <li class="nav-item">
                     ...
                     <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="./app/company-info.php">
                            <i class="icon">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="./app/tenant.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="./app/payment.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="./app/document.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="./app/expense_record.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="./app/tenant_property.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <li class="nav-item">
                        ...
                        <ul class="sub-nav collapse" id="utilities-error" data-bs-parent="#sidebar-menu">
                            <li class="nav-item">
                                <a class="nav-link " href="./app/room.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                            ...
                            <li class="nav-item">
                                <a class="nav-link " href="./app/floor.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                            ...
                            <li class="nav-item">
                                <a class="nav-link " href="./app/building.php">
                                    <i class="icon">
                                        <svg  class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                    ...
                    <li class="nav-item">
                        ...
                        <ul class="sub-nav collapse" id="sidebar-auth" data-bs-parent="#sidebar-menu">
                            <li class="nav-item">
                                <a class="nav-link" href="./app/room_cat.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                            ...
                            <li class="nav-item">
                                <a class="nav-link" href="./app/expense_type.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                            ...
                            <li class="nav-item">
                                <a class="nav-link" href="./app/document_cat.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                            ...
                            <li class="nav-item">
                                <a class="nav-link" href="./app/banks.php">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                            <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                    ...
                    <li class="nav-item">
                        <a class="nav-link"  href="./app/additional_fee">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M12 2L2 12h10v10h4V12h10L12 2z" fill="currentColor"></path>
                                    <path d="M12 8v8" stroke="white" stroke-width="2"></path>
                    ...
                    <li class="nav-item">
                        <a class="nav-link"  href="./app/penality.php">
                            <i class="icon">
                            <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M12 2L2 12h10v10h4V12h10L12 2z" fill="currentColor"></path>
                                </svg>
        ...
        <nav class="nav navbar navbar-expand-xl navbar-light iq-navbar">
          <div class="container-fluid navbar-inner">
            <a href="../dashboard/index.php" class="navbar-brand">
                ...
                <h4 class="logo-title">Building</h4>
            </a>
            <div class="sidebar-toggle" data-toggle="sidebar" data-active="true">
                <i class="icon">
    ...
    <marquee behavior="scroll" direction="left" scrollamount="5">
        Welcome to Building Management System Site
    </marquee>
...
<li class="nav-item dropdown mb--3">
    <a href="#" class="nav-link position-relative" id="notification-drop" data-bs-toggle="dropdown">
        ...
        <span class="bg-danger dots position-absolute top-0 start-100 translate-middle badge rounded-pill" id="notification-count" style="display: none;">0</span>
    </a>
    <div class="dropdown-menu dropdown-menu-end p-0" aria-labelledby="notification-drop">
                      ...
                      <div class="col-md-12">
                          <div class="flex-wrap d-flex justify-content-between align-items-center">
                              <div>
                                  ...
                                  <p>Welcome to Building Management System Site</p>
                              </div>
                              <!-- <div>
    ...
    <div class="card-body">
        <div class="progress-widget">
            ...
            <div class="progress-detail">
                ...
                <h4 class="counter"><?php echo $tenant_count; ?></h4>
            </div>
        </div>
    </div>
</li>
<li class="swiper-slide card card-slide" data-aos="fade-up" data-aos-delay="800">
    <div class="card-body">
        <div class="progress-widget">
            ...
            <div class="progress-detail">
                <p class="mb-2">This Month's Income</p>
                <h4 class="counter">Birr <?php echo number_format($this_month_income, 2); ?></h4>
            </div>
        </div>
    </div>
</li>
<li class="swiper-slide card card-slide" data-aos="fade-up" data-aos-delay="900">
    <div class="card-body">
        <div class="progress-widget">
            ...
            <div class="progress-detail">
                <p class="mb-2">This Month's Expense</p>
                <h4 class="counter">Birr <?php echo number_format($this_month_expense, 2); ?></h4>
            </div>
        </div>
    </div>
</li>
<li class="swiper-slide card card-slide" data-aos="fade-up" data-aos-delay="1000">
    <div class="card-body">
        <div class="progress-widget">
            ...
            <div class="progress-detail">
                <p class="mb-2">This Week's Receivables</p>
                <h4 class="counter">Birr <?php echo number_format($this_week_receivables, 2); ?></h4>
            </div>
        </div>
    </div>
</li>
   ...
   <div class="row">
          ...
          <div class="col-lg-4 col-md-6 mb-4"> <!-- Added col-md-6 for responsiveness -->
            <div class="card">
              <div class="card-header">
                <h5 class="mb-0">Tenant Growth Over Time</h5>
              </div>
              <div class="card-body">
                <div id="tenant-growth-chart"></div>
              </div>
            </div>
          </div>

          <!-- Monthly Payments Received (Bar Chart) -->
          <div class="col-lg-4 col-md-6 mb-4"> <!-- Added col-md-6 for responsiveness -->
            <div class="card">
              <div class="card-header">
                <h5 class="mb-0">Monthly Payments Received</h5>
              </div>
              <div class="card-body">
           ...
           <div class="col-lg-4 col-md-6 mb-4"> <!-- Added col-md-6 for responsiveness -->
            <div class="card text-center">
              <div class="card-body">
                ...
                <a href="./Report/report.php" class="btn btn-primary">View Report</a>
              </div>
            </div>
          </div>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        // Tenant Growth Over Time Data
        const tenantGrowthData = <?= json_encode($tenant_growth, JSON_HEX_TAG) ?>;
        const tenantGrowthMonths = <?= json_encode($tenant_months, JSON_HEX_TAG) ?>;

        // Monthly Payments Received Data
        const monthlyPaymentsData = <?= json_encode($monthly_payments, JSON_HEX_TAG) ?>;
        const paymentMonths = <?= json_encode($payment_months, JSON_HEX_TAG) ?>;

        // Revenue vs. Expenses Data
        const revenueData = <?= json_encode($revenue, JSON_HEX_TAG) ?>;
        const expensesData = <?= json_encode($expenses, JSON_HEX_TAG) ?>;
        const revenueExpenseMonths = <?= json_encode($revenue_expense_months, JSON_HEX_TAG) ?>;
        ...
        const tenantGrowthChart = new ApexCharts(document.querySelector("#tenant-growth-chart"), tenantGrowthOptions);
        tenantGrowthChart.render();
        ...
        const revenueExpensesOptions = {
          ...
          xaxis: {
            categories: revenueExpenseMonths,
            title: {
              text: 'Month'
            }
          },
        ...
        const revenueExpensesChart = new ApexCharts(document.querySelector("#revenue-expenses-chart"), revenueExpensesOptions);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchNotifications() {
        fetch('app/fetch_notifications.php')
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data.success && data.notifications.length > 0) {
                    ...
                    data.notifications.forEach(notification => {
                        ...
                        notificationItem.className = 'iq-sub-card' + (notification.status === 'unread' ? ' unread-notification' : '');
                        notificationItem.setAttribute('data-id', notification.id);
                        notificationItem.innerHTML = `
                            <div class="d-flex align-items-center">
                                <div class="ms-3 w-100">
                                    <h6 class="mb-0 text-start iq-text">${escapeHtml(notification.message)}</h6>
                                    <small class="float-end font-size-12">${new Date(notification.created_at).toLocaleString()}</small>
                                </div>
                            </div>
                        `;
                    ...
                    notificationList.innerHTML = '<p class="text-center p-3">No notifications available.</p>';
                    notificationCount.style.display = 'none';
                }
            })
            .catch(error => {
    ...
    // Optionally, set up periodic fetching (e.g., every 60 seconds)
