...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $sql = "INSERT INTO xionbms_users (username, fullname, email, role, password) VALUES (?, ?, ?, ?, ?)";
        ...
        if ($password) {
            $sql = "UPDATE xionbms_users SET username = ?, fullname = ?, email = ?, role = ?, password = ? WHERE ID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssi", $username, $fullname, $email, $role, $password, $id);
        } else {
            $sql = "UPDATE xionbms_users SET username = ?, fullname = ?, email = ?, role = ? WHERE ID = ?";
            $stmt = $conn->prepare($sql);
        ...
        $sql = "DELETE FROM xionbms_users WHERE ID = ?";
        $stmt = $conn->prepare($sql);
...
// Fetch users
$sql = "SELECT ID, username, email, fullname, role FROM xionbms_users";
...
<html lang="en" dir="ltr" data-bs-theme="light" data-bs-theme-color="theme-color-default">
  <head>
    <meta charset="utf-8">
      ...
      <title>Nest-Net | Responsive Bootstrap 5 Admin Dashboard Template</title>
...
@media (max-width: 768px) {
  ...
  <body class="  ">
    ...
    <div class="logo-mini">
      ...
      <main class="main-content">
        ...
        <div class="container-fluid">
            ...
            <!-- Add New User Button -->
            <?php if ($role === 'admin'): ?>
                <div style="margin-top: 20px;" class="d-flex justify-content-end mb-3">
        ...
        <tr data-id="<?= $user['ID'] ?>"> <!-- Added data-id here -->
            ...
            <td data-label="Actions">
                <?php if ($role === 'admin'): ?>
                    <button class="btn btn-sm btn-success edit-btn" data-id="<?= $user['ID'] ?>" data-bs-toggle="modal" data-bs-target="#editUserModal">
                ...
                <?php else: ?>
                    <button class="btn btn-sm btn-success edit-btn" data-id="<?= $user['ID'] ?>" data-bs-toggle="modal" data-bs-target="#editUserModal" disabled>
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger delete-btn" data-id="<?= $user['ID'] ?>" disabled>
                        <i class="fas fa-trash"></i>
                    </button>
                <?php endif; ?>
            </td>
                            ...
                            <form id="addUserForm">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="password" class="form-control" id="add-confirm-password" name="confirm-password" required>
                                    </div>
                                </div>
                            ...
                            <form id="editUserForm">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="password" class="form-control" id="edit-confirm-password" name="confirm-password" placeholder="Confirm new password">
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
    ...
    <a class="btn btn-fixed-end btn-warning btn-icon btn-setting" data-bs-toggle="offcanvas" data-bs-target="#offcanvasExample" role="button" aria-controls="offcanvasExample">
     ...
     <!-- <a class="btn btn-fixed-end btn-secondary btn-icon btn-dashboard z-3" href="../landing-pages/index.php" target="_blank">
      Landing Pages
    </a> -->
    <!-- Wrapper End-->
    <!-- offcanvas start -->
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    editUserModal.addEventListener('show.bs.modal', function (event) {
        ...
        // Extract user data from the button's data attributes or the table row
        var userId = button.getAttribute('data-id'); // Retrieve from button
        var tr = button.closest('tr'); // Table row

        // Ensure the table row exists
        if (!tr) {
            console.error('Cannot find the table row for the selected user.');
            return;
        }

        // Retrieve user details from the table cells
        var username = tr.querySelector('td[data-label="Username"]').innerText.trim();
        var email = tr.querySelector('td[data-label="Email"]').innerText.trim();
        var fullname = tr.querySelector('td[data-label="Full Name"]').innerText.trim();
        var role = tr.querySelector('td[data-label="Role"]').innerText.trim();
        ...
        modal.querySelector('#edit-confirm-password').value = '';
    });
    ...
    document.getElementById('addUserForm').addEventListener('submit', function (e) {
        ...
        .then(data => {
            if (data.trim() === 'success') {
                ...
                location.reload(); // Reload to see the new user in the table
            } else {
    ...
    document.getElementById('editUserForm').addEventListener('submit', function (e) {
        ...
        .then(data => {
            if (data.trim() === 'success') {
                ...
                location.reload(); // Reload to see the updated user data
            } else {
     ...
     document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function() {
            const userId = this.getAttribute('data-id'); // Retrieve from button
            if (confirm('Are you sure you want to delete this user?')) {
                // Create a FormData object for the delete action
